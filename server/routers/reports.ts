...
export const reportsRouter = router({
  upload: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Check whether any existing active report of this type exists (BEFORE superseding)
      ...
      const wasSuperseded = existingRows.length > 0;  // ← bug fix: was hardcoded true

      // Mark existing reports of same type as superseded
      if (wasSuperseded) {
  ...
  // ─── Server-side 12-week stress test ──────────────────────────────────────
  runStressTest: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const currentCash = bs?.assets.currentAssets.find(a => /cash/i.test(a.name))?.amount ?? 0;
...
${criticals.length > 0 ? `<div class="section"><h2>Critical Issues Requiring Immediate Action</h2>${criticals.map(a => `<div class="alert-box alert-critical"><div class="alert-title">${a.title}</div><div class="alert-detail">${a.detail}</div></div>`).join("")}</div>` : ""}
...
${ar && ar.totals.days91plus > 0 ? `<div class="section"><h2>A/R Aging — 91+ Day Balances</h2><table><thead><tr><th>Customer</th><th>91+ Days</th><th>Total</th></tr></thead><tbody>${ar.rows.filter(r => r.days91plus > 0).map(r => `<tr><td>${r.customer}</td><td style="color:#dc2626;font-weight:bold">$${r.days91plus.toLocaleString()}</td><td>$${r.total.toLocaleString()}</td></tr>`).join("")}</tbody></table></div>` : ""}
<div class="section">
  ...
  <ol style="font-family:sans-serif;font-size:13px;line-height:1.8;">
    ...
    ${criticals.length + warnings.length === 0 ? "<li>No critical actions required at this time. Continue monitoring key metrics.</li>" : ""}
  </ol>
</div>
<div class="footer">
...
function dispatchCSVParser(reportType: string, rows: Record<string, string>[]): unknown {
  switch (reportType) {
    case "BalanceSheet":    return parseBalanceSheet(rows.map(r => Object.values(r).join(" ")));
    case "ProfitLoss":      return parseProfitLoss(rows.map(r => Object.values(r).join(" ")));
...
function dispatchExcelParser(
  ...
  switch (reportType) {
    ...
    case "BalanceSheet":    return parseBalanceSheet(rawRows.map(r => Object.values(r).join(" ")));
    case "ProfitLoss":      return parseProfitLoss(rawRows.map(r => Object.values(r).join(" ")));
...
function dispatchTextParser(reportType: string, lines: string[]): unknown {
  switch (reportType) {
    case "BalanceSheet":    return parseBalanceSheet(lines);
    case "ProfitLoss":      return parseProfitLoss(lines);
