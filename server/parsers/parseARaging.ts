...
export function parseARaging(rows: Record<string, string>[]): ARAgingData {
  ...
  let totals: ARAgingRow = { customer: 'TOTAL', current: 0, days1to30: 0, days31to60: 0, days61to90: 0, days91plus: 0, total: 0, isTotal: true };

  for (const row of rows) {
    const name = row['Customer'] ?? row['Name'] ?? row['customer'] ?? '';
    if (!name) continue;
    ...
    const agingRow: ARAgingRow = {
      ...
      days1to30: parseQBOAmount(row['1 - 30'] ?? row['1-30'] ?? row['31-60'] ?? '0'),
